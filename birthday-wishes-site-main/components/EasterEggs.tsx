  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-30">
      <button
        aria-label="moon"
        tabIndex={0}
        className={`easter-egg-trigger ${phase === 2 ? 'easter-egg-trigger-dark' : ''} pointer-events-auto fixed right-6 top-6 ${hintTarget === 'moon' ? 'animate-hint-zoom' : ''}`}
        onClick={() => {
          setMoonMessage(true)
          markFound('moon')
        }}
      ...
      <button
        ...
        tabIndex={0}
        className={`easter-egg-trigger ${phase === 2 ? 'easter-egg-trigger-dark' : ''} pointer-events-auto fixed left-6 top-6 ${hintTarget === 'heart' ? 'animate-heart-squeeze' : ''}`}
